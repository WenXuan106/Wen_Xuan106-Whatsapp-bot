...
async function startSocket({ phoneNumber, forceReset = false } = {}) {
  ...
  const newSock = makeWASocket({
    ...
    // Default query timeout is tuned for normal hosting. Group message
    // sending has to fetch + process encryption prekeys for every member
    // who doesn't already have a session — potentially dozens of people
    // in one request — which is far more work than a DM's single party.
    // Under Render free's throttled shared CPU, that can legitimately
    // take longer than the default allows, surfacing as "Timed Out"
    // specifically for groups even though DMs work fine. Give it more
    // room rather than failing early.
    defaultQueryTimeoutMs: 120000,
    // Skip syncing chat history altogether — nothing in this bot reads or
    // stores it (no message store, no listener for it), so there's no
    // reason to spend time/bandwidth pulling it down, groups included.
    // Baileys doesn't offer a "skip groups only" toggle here, but since
    // it's unused either way, disabling it entirely is the correct call.
    shouldSyncHistoryMessage: () => false,
    // IMPORTANT: this must actually fetch-and-cache on a miss, not just
    // read the cache. Baileys needs the participant list from here to
    // know who to encrypt a group message for — returning undefined on
    // a cold cache (e.g. right after a fresh pairing, or any group it
    // hasn't seen yet) meant it had no participants to establish
    // sessions with at all, which surfaced as "SessionError: No
    // sessions" for every group message, independent of session state.
  ...
  newSock.ev.on("messages.upsert", async ({ messages, type }) => {
    // Safety net: nothing in this handler — including the error-reporting
    // fallback below — should ever be able to crash the whole process.
    // A broken group signal-session can make even the "something went
    // wrong" message itself fail to send; without this, that second
    // failure was an unhandled rejection that took the entire bot down.
    try {
      ...
      if (jid.endsWith("@g.us") && !msg.key.fromMe) {
        ...
        // Same idea as civilguard above — hangman/tictactoe/scramble/math
        // need to see plain guesses/moves ("a", "5", "computer"), not just
        // "!" commands.
        try {
          const senderJid = msg.key.participant || msg.key.remoteJid;

          const hangmanHandled = await handleHangmanGuess({ sock: newSock, jid, text });
          if (hangmanHandled) return;

          const tttHandled = await handleTicTacToeMove({ sock: newSock, jid, senderId: senderJid, text });
          if (tttHandled) return;

          const scrambleHandled = await handleScrambleGuess({ sock: newSock, jid, text });
          if (scrambleHandled) return;

          const mathHandled = await handleMathAnswer({ sock: newSock, jid, text });
      ...
      try {
        ...
        // This send can itself fail (e.g. broken/missing signal session for
        // this chat) — that must not escape as a second, unhandled error.
        try {
