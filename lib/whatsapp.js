...
async function startSocket({ phoneNumber, forceReset = false } = {}) {
  ...
  newSock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    // We hand Baileys `cachedGroupMetadata` above so it doesn't refetch
    // participants on every group message — but that means Baileys never
    // fetches metadata itself either. If a group's metadata isn't already
    // in our cache (e.g. bot just restarted, or this is the first message
    // from this group), `cachedGroupMetadata` resolves to undefined and
    // Baileys has no participant list to encrypt outgoing messages for —
    // sendMessage to that group then silently fails. Warm the cache here,
    // for every group message, before any command can try to reply.
    // getGroupMetadata() is a no-op API call when the cache is still warm
    // (TTL check), so this doesn't reintroduce the per-message fetch cost.
    if (jid.endsWith("@g.us")) {
      try {
        await getGroupMetadata(newSock, jid);
      } catch (err) {
        console.error("Failed to warm group metadata cache:", err.message);
      }
    }
