...
const { handleJoin: handleWelcomeJoin } = require("./welcome");
...
async function startSocket({ phoneNumber, forceReset = false } = {}) {
  ...
  newSock.ev.on("group-participants.update", async ({ id, participants, action }) => {
    try {
    ...
    if (action === "add" && participants?.length) {
      try {
        await handleWelcomeJoin({ sock: newSock, jid: id, participants });
      } catch (err) {
  ...
  newSock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    // TicTacToe moves are plain numbers/"surrender", not prefixed commands,
    // so — like civilguard above — this has to see the message before the
    // prefix check below would otherwise discard it.
    if (jid.endsWith("@g.us") && !msg.key.fromMe) {
      try {
        const moveSenderJid = msg.key.participant || msg.key.remoteJid;
        const consumed = await handleTicTacToeMove({ sock: newSock, jid, senderId: moveSenderJid, text });
        if (consumed) return;
      } catch (err) {
    ...
    // Same idea for Hangman single-letter guesses.
    if (jid.endsWith("@g.us") && !msg.key.fromMe) {
      try {
        const consumed = await handleHangmanGuess({ sock: newSock, jid, text });
        if (consumed) return;
      } catch (err) {
    ...
    // Tally message activity per group for !topmembers. Counts every
    // ordinary group message, not just commands.
    if (jid.endsWith("@g.us") && !msg.key.fromMe) {
      try {
        const authorJid = msg.key.participant || msg.key.remoteJid;
        incrementCount(jid, authorJid);
      } catch (err) {
        console.error("topmembers count error:", err);
      }
    }
    ...
    const senderJid = msg.key.participant || msg.key.remoteJid;
    if (!msg.key.fromMe && isBanned(senderJid)) return;
