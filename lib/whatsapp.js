...
async function startSocket({ phoneNumber, forceReset = false } = {}) {
  ...
  newSock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    try {
      ...
      // Text of whatever message this one is replying to, if any — lets
      // commands like !translate work on a quoted message instead of
      // requiring the text to be retyped.
