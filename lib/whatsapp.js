...
async function startSocket({ phoneNumber, forceReset = false } = {}) {
  ...
  const newSock = makeWASocket({
    ...
    // Under a throttled shared-CPU host, that can legitimately take
    // longer than the default allows. Give it more room rather than
    // failing early.
    ...
    // a cold cache meant it had no participants to establish sessions
    // with at all, which surfaced as "SessionError: No sessions" for
    // every group message, independent of session state.
