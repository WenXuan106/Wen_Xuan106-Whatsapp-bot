...
function rainLines(cx, cy, s, count = 3, fill = COLORS.white) {
  let out = "";
...
function buildWeatherCardSvg(data) {
  ...
  const titleSize = fitFontSize(titleText, innerWidth, 52, 30);

  const dayCount = Math.max(1, data.outlookDays.length);
  const gap = 24;
  const dayCardWidth = (innerWidth - gap * (dayCount - 1)) / dayCount;
  const dayCardHeight = 220;
  const dayCardsTop = 700;

  const height = data.outlookDays.length ? dayCardsTop + dayCardHeight + 90 : 560;

  let svg = `<svg width="${CARD_WIDTH}" height="${height}" viewBox="0 0 ${CARD_WIDTH} ${height}" xmlns="http://www.w3.org/2000/svg">`;
  ...
  svg += `
    ...
    <rect x="4" y="4" width="${CARD_WIDTH - 8}" height="${height - 8}" rx="36" fill="url(#bg)" stroke="${COLORS.border}" stroke-width="2"/>
  `;

  // Title + location
  svg += `<text x="${pad}" y="120" font-family="sans-serif" font-weight="700" font-size="${titleSize}" fill="${COLORS.accent}">${esc(titleText)}</text>`;
  svg += `<text x="${pad}" y="168" font-family="sans-serif" font-weight="700" font-size="34" fill="${COLORS.white}">${esc(data.location)}</text>`;

  // Icon + big temp + condition
  const iconBoxY = 210;
  svg += weatherIcon(data.condition, pad, iconBoxY, 130);
  svg += `<text x="${pad + 170}" y="${iconBoxY + 95}" font-family="sans-serif" font-weight="800" font-size="100" fill="${COLORS.white}">${Math.round(data.tempC)}°C</text>`;
  svg += raindropMini(pad + 190, iconBoxY + 135, 0.7);
  svg += `<text x="${pad + 210}" y="${iconBoxY + 143}" font-family="sans-serif" font-size="30" fill="${COLORS.muted}">${esc(data.description)}</text>`;

  // Stats — two rows of three
  const statRow1Y = iconBoxY + 210;
  const statRow2Y = statRow1Y + 55;
  const colX = [pad, pad + innerWidth / 3, pad + (2 * innerWidth) / 3];

  const stat = (x, y, iconFn, label) => {
    let out = iconFn(x + 16, y - 6, 1);
    out += `<text x="${x + 40}" y="${y}" font-family="sans-serif" font-size="27" fill="${COLORS.white}">${esc(label)}</text>`;
    return out;
  };
  ...
  // Divider
  const dividerY = statRow2Y + 40;
  svg += `<line x1="${pad}" y1="${dividerY}" x2="${CARD_WIDTH - pad}" y2="${dividerY}" stroke="${COLORS.cardBorder}" stroke-width="2"/>`;

  // 3-day outlook header
  const outlookHeaderY = dividerY + 60;
  svg += calendarIcon(pad + 16, outlookHeaderY - 10, 1);
  svg += `<text x="${pad + 40}" y="${outlookHeaderY}" font-family="sans-serif" font-weight="700" font-size="36" fill="${COLORS.accent}">3-DAY OUTLOOK</text>`;

  // Day cards
  data.outlookDays.forEach((day, i) => {
    const x = pad + i * (dayCardWidth + gap);
    const y = dayCardsTop;
    svg += `<rect x="${x}" y="${y}" width="${dayCardWidth}" height="${dayCardHeight}" rx="20" fill="${COLORS.cardBg}" stroke="${COLORS.cardBorder}" stroke-width="2"/>`;
    svg += `<text x="${x + 24}" y="${y + 46}" font-family="sans-serif" font-weight="700" font-size="30" fill="${COLORS.white}">${esc(day.label)}</text>`;
    svg += weatherIcon(day.condition, x + 20, y + 60, 60);
    svg += `<text x="${x + 96}" y="${y + 100}" font-family="sans-serif" font-weight="700" font-size="30" fill="${COLORS.white}">${day.max}° / ${day.min}°</text>`;
    svg += raindropMini(x + 32, y + 148, 0.6);
    svg += `<text x="${x + 50}" y="${y + 154}" font-family="sans-serif" font-size="22" fill="${COLORS.muted}">${esc(day.description)}</text>`;
    svg += raindropMini(x + 32, y + 182, 0.6);
    svg += `<text x="${x + 50}" y="${y + 188}" font-family="sans-serif" font-size="22" fill="${COLORS.muted}">Rain: ${day.pop}%</text>`;
  });

  // Footer
  const footerY = height - 40;
  svg += shieldIcon(pad + 12, footerY - 6, 1);
  svg += `<text x="${pad + 34}" y="${footerY}" font-family="sans-serif" font-size="24" fill="${COLORS.muted}">Powered by ${esc(data.botName)}</text>`;
